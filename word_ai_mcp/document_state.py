...
TOOLS = {
    ...
    "word_session_preview": ("Prepare one edit without writing. operation: replace_text, replace_paragraph, insert_before, insert_after, set_style, delete_paragraph, format_text, list_restart, list_type, list_level. Supply expected_sha256 from read/snapshot. find is a unique literal (max 255 chars) for replace_text and format_text (empty find formats the whole paragraph), otherwise empty. text is one line; set_style: Normal, Title, Subtitle or Heading1..Heading9; format_text: comma-separated bold, italic, underline, no_bold, no_italic, no_underline; list_type: bullet or number (whole list level); list_level: 0..8; delete_paragraph and list_restart (numbering from 1 at this item; the preview reports method start_override (Word's Restart at 1 on level 0; the item gets a new paragraph_id) or set_starting_number (first item of a deeper level); check scope and renumbered_item_count): empty. Returns preview_id via command_status.", {"session_id": "string", "paragraph_id": "string", "operation": "string", "text": "string", "find": "string", "expected_sha256": "string"}),
    ...
    "word_session_command_status": ("Get command result. expired/cancelled means not started; failed with result.unchanged=true means Word rejected the write and the document was verified unchanged; unknown requires inspection, never automatic retry.", {"session_id": "string", "command_id": "string"}),
...
class State:
    ...
    def office(self, route, body, token=""):
        with self.lock:
            ...
            # unchanged: Word rejected the submitted write and the panel's fresh
            # read matched the preview, so nothing was written and the session stays usable.
            written = result.get("submitted") is not False and result.get("unchanged") is not True
            c["status"] = "succeeded" if ok else ("unknown" if c["type"] == "apply" and written else "failed")
            c["result"] = result
